...
type BlogCardProps = Pick<BlogPost, "title" | "excerpt" | "date" | "image" | "slug" | "tags" | "author"> & {
  cmsSource?: CmsSourceConfig;
  priority?: boolean;
};

function BlogCard({ title, excerpt, date, image, slug, tags, author, cmsSource, priority = false }: BlogCardProps) {
      ...
      <Link to={`/blog/${slug}`} aria-label={`Buka artikel: ${title}`} className="flex h-full w-full flex-col">
        ...
        <div className="flex flex-1 flex-col space-y-3 p-5">
          <div className="flex items-center justify-between gap-3">
            <h2 className="line-clamp-2 text-base font-semibold leading-snug text-dark dark:text-dark-text">{title}</h2>
          </div>
          <p className="line-clamp-3 flex-1 text-sm leading-relaxed text-dark/70 dark:text-dark-text/70">{excerpt}</p>
          ...
          <div className="mt-auto flex items-center justify-between gap-3 border-t border-tan/20 pt-3 text-[11px] text-dark/50 dark:border-dark-bg-light/30 dark:text-dark-text/50">
            ...
            {author ? <p className="truncate text-right text-xs font-semibold text-dark/70 dark:text-dark-text/70">{author}</p> : null}
          </div>
        </div>
      </Link>
    </article>
